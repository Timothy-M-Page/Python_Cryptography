# K_Values are the first 16 hex characters of the decimal expansions of the
...
def pad_and_block(message: bytes) -> list[list[int]]:
    ...
    and adding a 128 bit representation of the length of the initial message
    such that the padded message length is a multiple of 128 bytes.
    """
    padded = message + b'\x80'
    padded += b'\x00' * ((112 - len(padded) % 128) % 128)
    padded += (len(message) * 8).to_bytes(16, "big")

    blocks = [[int.from_bytes(padded[i+j:i+j+8], "big")
               for j in range(0, 128, 8)] for i in range(0, len(padded), 128)]

    return blocks


def right_rotate_64(x: int, n: int) -> int:
...
def extend_words(words: list[int]) -> list[int]:
    for i in range(16, 80):
        new_word = (sigma1(words[i-2]) + words[i-7]
                    + sigma0(words[i-15]) + words[i-16]) & 0xFFFFFFFFFFFFFFFF
...
def sha2_512(message: bytes) -> str:
    ...
    for block in blocks:
        ...
        for i in range(80):
            temp1 = (h + sum1(e) + choice(e, f, g) + K_VALUES[i]
                     + message_schedule[i]) & 0xFFFFFFFFFFFFFFFF
            temp2 = (sum0(a) + majority(a, b, c)) & 0xFFFFFFFFFFFFFFFF
    ...
    return ''.join(f"{h:016x}" for h in [h0, h1, h2, h3, h4, h5, h6, h7])
