...
def sha2(message: bytes, length: int) -> str:
    ...
    if length == 224:
        ...
        raise ValueError("Unsupported SHA2 length.")
    ...
    if length == 224:
        return ''.join(f"{h:08x}" for h in [h0, h1, h2, h3, h4, h5, h6])
    if length == 256:
        return ''.join(f"{h & 0xFFFFFFFF:08x}" for h in
