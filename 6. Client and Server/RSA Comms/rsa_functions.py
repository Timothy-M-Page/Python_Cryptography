...
def oaep_encode(message: bytes, modulus_size: int) -> bytes:
    ...
    Note the modulus size in the number of bytes in n = prime1 * prime2
    """
    empty_hash = hashlib.sha256(b"").digest()
    hash_size = hashlib.sha256().digest_size

    zero_padding = b'\x00' * (modulus_size - len(message) - (2 * hash_size) - 2)
...
def hmac_sha256(key: bytes, message: bytes) -> bytes:
    block_size = 64  # SHA256 block size in bytes
...
def key_derivation_function(seed: bytes, byte_length: int = 32) -> bytes:
    """
    Derive a symmetric key from the seed using HKDF-SHA256.
    Default output length = 32 bytes (AES-256 bit key).
    """

    salt = b"rsa-kem"
    info = b"aes-key"

    prk = hmac_sha256(salt, seed)
...
def create_and_encapsulate_key(public_exponent: int, modulus: int) -> tuple[bytes, int]:

    key_seed = secrets.token_bytes(32)
    encapsulated_seed = rsa_encrypt(key_seed, public_exponent, modulus)

    aes_key = key_derivation_function(key_seed)

    return aes_key, encapsulated_seed


def decapsulate_and_create_key(cipher_seed: int, prime1: int, prime2: int, private_exponent: int, modulus: int) -> bytes:
    key_seed = rsa_decrypt(cipher_seed, prime1, prime2, private_exponent, modulus)
    aes_key = key_derivation_function(key_seed)
