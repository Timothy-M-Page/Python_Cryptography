...
# Server key pair
server_private_key = secrets.randbelow(point_order - 1) + 1
server_public_key = elliptic_curves.point_exponent(CURVE_PARAMETERS, base_point, server_private_key)

print("Keys generated:")
print(f"server_private_key : {server_public_key}")
print(f"server_public_key : {server_public_key}")

# Send server public key
connection.sendall(point_to_bytes(server_public_key))

print()
print("Sent server_public_key to client.")
print()

# Receive client public key
client_data = connection.recv(1024)
client_public_key = bytes_to_point(client_data)

print("Received client_public_key.")
print(f"client_public_key : {client_public_key}")
print()

server_shared_point = elliptic_curves.point_exponent(CURVE_PARAMETERS, client_public_key, server_private_key)

print("Shared elliptic curve co-ordinates established.")
print(f"Shared_point : {server_shared_point}")
print()

aes_key = derive_aes_key(server_shared_point[0])
...
while True:
    ...
    message_format.send_packet(connection, num_used_once, tag, ciphertext)
